...
> workdir/logs/yieldsWZ_ZZacc.txt
for iproc in ${PROCS[@]}; do
  proc=$iproc
  echo $proc >> workdir/logs/yieldsWZ_ZZacc.txt
  ...
  for iwzstep in ${WZSTEPs[@]}; do
    ...
    #echo "STEP"$wzstep >> workdir/logs/yieldsWZ_ZZacc.txt
	...
	for iflav in ${FLAVs[@]}; do  
	  ...
	  for lhe in ${LHEs[@]}; do
	      ...
	      root -q -l -b display/cards/listyieldsWZ_acc.C\(\"${file}\",${wzstep},\"${proc}\",${var},${lhe}\) >> workdir/logs/yieldsWZ_ZZacc.txt
              ilhe=`echo $ilhe +1 | bc`
...
sed -i '/Processing/d' workdir/logs/yieldsWZ_ZZacc.txt
sed -i '/^$/d' workdir/logs/yieldsWZ_ZZacc.txt
